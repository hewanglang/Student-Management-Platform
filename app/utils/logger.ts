...
export async function logAction(action: string, details?: string, userId?: string) {
    try {
        ...
        if (!response.ok) {
            // 不尝试读取响应体，只检查状态码
            console.error(`记录日志HTTP错误! 状态码: ${response.status}`);
            return;
        }
    } catch (error) {
...
export async function logApiError(
    ...
    try {
        ...
        await logAction(action, `失败: ${errorDetails}`, userId);
    } catch (logError) {
...
export async function logApiSuccess(
    ...
    try {
        await logAction(action, `成功: ${details || ''}`, userId);
    } catch (logError) {
        console.error('记录API成功日志失败:', logError);
    }
} 